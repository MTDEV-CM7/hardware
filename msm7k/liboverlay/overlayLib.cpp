...
static int get_mdp_format(int format) {
    ...
    case OVERLAY_FORMAT_YCrCb_420_SP:
        return MDP_Y_CRCB_H2V2;
    case OVERLAY_FORMAT_YCrCb_420_SP_TILE:
...
bool Overlay::startChannel(int w, int h, int format, int fbnum, bool norot) {
    mChannelUP = objOvCtrlChannel.startControlChannel(w, h, format, fbnum, norot);
    if (!mChannelUP) {
        return mChannelUP;
    }
    return objOvDataChannel.startDataChannel(objOvCtrlChannel, fbnum, norot);
}
...
bool OverlayControlChannel::startControlChannel(int w, int h,
    ...
    fb_var_screeninfo vinfo;
    int hw_format;
    hw_format = get_mdp_format(format);
    if (hw_format < 0) {
    ...
    if (!setOverlayInformation(w, h, hw_format))
        return false;
...
bool OverlayDataChannel::startDataChannel(
    ...
    if (objOvCtrlChannel.getOvSessionID(ovid) &&
        ...
        return startDataChannel(ovid, rotid, size, fbnum, norot);
    }
    else
        return false;
}

bool OverlayDataChannel::openDevices(int fbnum) {
    if (fbnum < 0)
        return false;
    char const * const device_template =
                      "/dev/graphics/fb%u";
